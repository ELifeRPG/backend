...
/// *.Application per ARCHITECTURE.md §9e) can live in this module instead of in Characters.Api,
/// which must not reference Accounts.Application directly. Implementations should fail closed
/// (throw) if the claim is missing rather than degrade — every endpoint that resolves this already
/// requires a gameserver:* scope, so a missing claim means something is misconfigured.
/// </summary>
