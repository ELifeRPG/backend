...
- **Cross-module ID references** (e.g. a `BankAccount` in `Banking` needs to reference the `AccountId`/`CharacterId` that `Account`/`Characters` owns) go through a small shared-kernel project (e.g. `Shared.Kernel`) containing **only** `StronglyTypedId`-based ID value types and the common `AggregateBase`/`DomainEvent` base types — never full entities or aggregates. Validating that an ID is real (not just well-formed) happens via a Mediator query into the owning module, not a cross-module join.
...
{
    public record Created(AccountId AccountId, string KeycloakUsername);
