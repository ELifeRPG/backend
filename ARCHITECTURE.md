...
Server-side Enfusion script. Talks only to the local Bridge Service (localhost HTTP, `:5200`), never directly to the Central API. This keeps secrets and HTTP resilience logic (retries, batching, offline buffering) out of the mod's sandbox and out of the PBO package. It also isolates an engine quirk: Enfusion's `RestContext` forces `Content-Type: application/x-www-form-urlencoded` on every outbound request regardless of the body, and the Bridge's local listener absorbs that so the Central API only ever sees normal JSON — see [docs/bridge.md](./docs/bridge.md).

### 3.2 Bridge Service

An **ASP.NET Core minimal-API application** running alongside the dedicated server process, listening on `:5200`. It is a web app rather than a background service because the Reforger mod calls it: the mod has no credentials and no HTTP resilience of its own, so it POSTs to this local listener and the Bridge does the rest (see §3.1). *(Corrected 2026-08-28: this section previously described a ".NET Worker Service"; verified against `eliferpg-reforger-bridge`, it is a `WebApplication`. The local listener also absorbs the Enfusion engine's forced `Content-Type: application/x-www-form-urlencoded` and re-issues proper JSON upstream, so the Central API never grows a form binder — see [docs/bridge.md](./docs/bridge.md).)*

- Uses `IHttpClientFactory` + `AddStandardResilienceHandler()` (Polly) on its outbound clients for per-call retries and circuit breaking.
...
- **Must** buffer outbound writes to durable local storage (e.g. SQLite) when the Central API is unreachable, flushing once connectivity is restored. *(Corrected 2026-08-28: this bullet previously read as a description of shipped behaviour. Verified against `eliferpg-reforger-bridge`: there is no SQLite, no queue, no outbox and no replay — only the per-call resilience handler above, which survives a flaky connection and loses everything to a process restart. Every write on the inventory path reports something that already happened in the game, so a lost request is a lost fact, not a retryable action. [docs/bridge.md](./docs/bridge.md) specifies the requirement and what the Central API guarantees in return: `{gameServerId}:{batchId}` idempotency within the retention window, per-instance revision last-write-wins outside it, and a `retryable` flag on every error.)*
- Batches outbound writes — one request per batch rather than one call per change. There is no single generic `POST /api/events/batch` route; each module owns its own typed batched endpoint, `POST /api/inventory/snapshots` being the one built (see §5.1's note and [docs/bridge.md](./docs/bridge.md)).
...
- Exposes each module's own batched write endpoints (see §5.1's note — there is no single generic event-ingestion route).
...
- Will expose REST endpoints for catch-up/replay of the consumer event stream described in §5.3 (`GET /events?since=<sequence>`) — **not built; no such route exists today**, see §5.1's note.
...
- Bridge sends batched writes to the Central API, one request per batch rather than one per change.
- Each batch carries a client-generated GUID for idempotency (deduplication on retry).
- The Central API validates scope, schema, and per-server rate limits before persisting.

**Note on `POST /api/events/batch` (corrected 2026-08-28):** the three bullets above described a single generic event-ingestion endpoint, and **no such route exists** — verified against `src/` and against the generated `openapi/eliferpg-api-v1.json`, which contains no path matching `event`. The design went a different way: rather than one untyped event firehose, each module exposes its own typed write endpoints, and the batching/idempotency properties this section describes are real but live on those. The concrete instance of this pattern today is the World module's `POST /api/inventory/snapshots` — batched, one client-generated `batchId` per batch for idempotency (keyed on `{gameServerId}:{batchId}`, see [docs/bridge.md](./docs/bridge.md#ordering-idempotency-and-replay)), scope- and schema-validated, and rate-limited per gameserver as described below. A generic event stream may still land for the SignalR/NPC consumer path in §5.3, which is what the rest of this section is written against; when it does, it should adopt the same three properties rather than reinvent them.

**Concretely, as built (2026-08-28):** rate limiting is host-level plumbing (`AddRateLimiter`/`UseRateLimiter` in `src/Api/Program.cs`) with **no endpoint limited by default**. Each module registers its own named policies beside its authorization policies and opts endpoints in with `RequireRateLimiting`, the same way it opts them into a scope. The World module is the first to do so, with token buckets partitioned on the caller's `client_id` claim — one bucket per gameserver, since one Keycloak client per gameserver instance is already the credential model (§4.2). The host owns only the shared shape of a rejection: `429` with a `Retry-After` header and `retryable: true` in the problem document, which is the pair a store-and-forward client needs to tell "wait" from "drop this". The enforced burst and sustained rate are published on `GET /api/inventory/limits` so no client hardcodes them — see [docs/bridge.md](./docs/bridge.md#rate-limits).
...
- SignalR is a delivery convenience, not the source of truth: every consumer tracks the last processed sequence number and catches up via `GET /events?since=<sequence>` after a disconnect or restart. **Neither the hub nor that route is built yet** — this section is the design for the consumer path, not a description of it; see §5.1's note.
...
| Resilience (Bridge) | Polly, via `AddStandardResilienceHandler()` on outbound clients — **per-call retry/circuit breaking only**; durable offline buffering is a requirement on the Bridge, not something that ships today (§3.2, [docs/bridge.md](./docs/bridge.md)) | BSD-3 |
| Bridge runtime | ASP.NET Core minimal API (listens on `:5200`) | MIT |
| Admin UI | Vue | MIT |
...
- **Later:** **Kubernetes** is the anticipated next step once there's a real fleet of gameserver hosts and/or a need for independent scaling of the Central API, SignalR fan-out, and the observability stack. To keep that migration low-friction:
  ...
  - The Bridge Service is explicitly **not** containerized alongside the gameserver host's orchestration story — it runs as a plain process next to the dedicated server binary, since gameserver hosts are unlikely to be Kubernetes nodes themselves.
...
- **Schema collapse on short type name.** `Microsoft.Extensions.ApiDescription.Server` keys generated OpenAPI schema components on a DTO's short type name, not its full namespace. Two modules' `*.Api` projects are allowed — expected, per the DTO-ownership rule in [§9e](#9e-modulith-structure--module-boundaries) — to each define their own identically-named DTO for the same wire shape (`World.Api`/`Shops.Api` both have a `GrantedInstanceDto`; `World.Api`/`Characters.Api` both have a `SkillXpGrantDto`), and the generator silently folds each pair into one shared schema in `openapi/eliferpg-api-v1.json`. Harmless while both sides stay structurally identical, but the moment one diverges the generator must either rename a schema (breaking the Kiota-generated Bridge client, which names its model classes off that schema name) or emit a wrong shared shape for one of the two endpoints. See `docs/world.md` for the concrete pair and what to check before assuming a single-module DTO change is isolated.
...
The Bridge is **not** in this tree: it is a separate ASP.NET Core app in its own repository, `eliferpg-reforger-bridge` (§3.2, §9b), consuming this API over HTTP through a Kiota-generated client.
...
9. **A `SessionOptions.ForTransaction`-bound session cannot use Marten's version-checked event-append machinery — neither `FetchForWriting` nor an explicit-version `Append` — even though both work fine on an ordinary scoped/lightweight session.** Version tracking for a stream is broken at the Marten/JasperFx level on a session opened via `SessionOptions.ForTransaction(existingTransaction, shouldAutoCommit: false)` (the shape every cross-module writer in this codebase uses to join a shared `ICrossModuleTransaction`'s `NpgsqlTransaction`), so relying on it to serialize two concurrent writers racing the same aggregate silently stops working the moment a handler is migrated onto `ICrossModuleTransaction` — no exception, no compile-time signal, just a lost concurrency guarantee. Verified against live Postgres via 3 spikes while migrating `Shops.Application.Shops.PurchaseListingCommand` off its saga onto `ICrossModuleTransaction` (2 NO-GO — `FetchForWriting` and explicit-version `Append` each independently confirmed broken on a `ForTransaction`-bound session — then 1 GO for the workaround below). **The verified workaround:** take a Postgres-native row lock as raw SQL against the shared `NpgsqlTransaction` — `SELECT id FROM <schema>.mt_doc_<aggregate> WHERE id = @id FOR UPDATE` (filter the doc table's primary key, or the query seq-scans the table) — *before* loading/mutating the aggregate, held until the shared transaction commits or rolls back, followed by a plain, unversioned `session.Events.Append(id, domainEvent)`. The row lock itself (not Marten) is what gives "two concurrent writers can never both succeed." See `MartenShopListingRepository.ReserveStockAsync` for the reference implementation and `tests/Shops.IntegrationTests/CrossModuleRowLockSpikeTests.cs` for the spike that proved it out. **Updated 2026-08-22:** the predicate was originally `WHERE tenant_id = @tenant AND id = @id`, filtering both columns of the doc table's composite `(tenant_id, id)` primary key — conjoined multi-tenancy made `tenant_id` part of the key, so both columns had to be filtered together or the query would seq-scan the table. Removing conjoined tenancy from Banking, Companies, and Shops (the last three modules that still had it) dropped `tenant_id` from the key entirely, leaving `id` alone as the primary key, confirmed live for all three: `CREATE UNIQUE INDEX pkey_mt_doc_bankaccount_id ON banking.mt_doc_bankaccount USING btree (id)` (and the equivalent `pkey_mt_doc_company_id`/`pkey_mt_doc_shoplisting_id` on the other two). `WHERE id = @id` is therefore an exact leading-column match on a unique index — satisfied by construction, a stronger guarantee than any single `EXPLAIN` run. Caveat for whoever next runs `EXPLAIN` against a small/freshly-seeded table and sees `Seq Scan`: that's expected, not a regression — on the small tables seeded during this migration (35-98 rows) Postgres's cost-based planner correctly preferred a seq scan regardless of the index, and the index path itself was only confirmed by forcing it with `SET enable_seqscan = off`. The underlying point survives unchanged: filter the doc table's full primary key, whatever it currently is, or risk a seq-scan on a table large enough for the planner to care.

10. **`session.Store(document)` and `session.Delete<T>(id)` for the *same id* in one `SaveChangesAsync` silently drop the `Store()`'s field changes — the delete wins, with no error, no exception, nothing in the logs.** Marten's unit of work keeps only the last-registered document-storage operation per id, so queuing a `Store()` to update a field and a `Delete()` to remove the row (in either order) collapses to just the delete; whatever the `Store()` changed never reaches Postgres. Discovered building `World`'s delivery loop: the rule "an explicit delete of a pending row clears its `PendingSpawn` flag as it deletes" needs both a field write and a row delete on the same `ItemInstance.Id` in one commit, and the naive `Store()`+`Delete()` pairing silently defeats it — the flag would appear cleared in memory (the object the caller holds) but the deleted row would carry the old value into Postgres, and a later undo/restore of that soft-deleted row would resurrect it still `PendingSpawn = true`, re-offering (and re-spawning) an item that was already delivered. **The workaround: use `session.Patch<T>(id).Set(...)` instead of `Store()` alongside the `Delete()`** — `Patch()` is tracked as a distinct operation kind from document storage, so it survives in the same `SaveChangesAsync` rather than being collapsed away. See `MartenItemInstanceRepository.SoftDelete` and `ItemInstanceRepositoryTests.ExplicitDelete_OfAPendingInstance_ClearsPendingAsItDeletes` (verified against real Postgres) for the reference fix. This generalizes beyond `World`: any handler that both mutates and deletes the same document in one commit needs `Patch()`, not `Store()`, for the mutation.

**`World`'s `ItemInstance` is a plain Marten document, never a projection — a deliberate departure from this repo's "durable auditable facts are events" rule.** Every other aggregate in this codebase (`Account`, `BankAccount`, `Company`, …) is event-sourced; `ItemInstance` isn't, because the mod reports a *snapshot*, not a delta — under last-write-wins the game sends the full current state of an instance on every upsert, so event-sourcing it would produce nothing but `StateReplaced` events carrying the whole document each time, none of event sourcing's actual benefits (no meaningful domain events to replay, no incremental history), and roughly double the write volume for no gain. Full-world persistence also eventually needs pruning (expired ground items, old movements) — and a pruned event stream permanently breaks projection rebuild, which an aggregate this codebase might someday need to rebuild from scratch cannot tolerate. The audit trail that would otherwise live in the event stream is intended to live in a separate, independently-prunable `ItemMovement` document — **which does not exist yet**: no such type is in `src/` as of phase 2, and the movement audit is listed against a later phase in [docs/world.md](./docs/world.md#scopes). Until it lands, there is no audit trail for an instance's history beyond its current state, which is a known gap rather than a shipped mechanism. A guard test, `WorldStoreTests.ItemInstance_IsNotRegisteredAsAProjection`, enforces that `World.Infrastructure`'s Marten store never registers a projection for it — so the next reviewer who notices `ItemInstance` isn't event-sourced and reaches for "fix" finds the reasoning and the regression test in the same place instead of re-litigating it. See `docs/world.md` for the module's read/write surface.

**`World` is one module deliberately holding two stores with different tenancy — hive-wide today, per-server later.** `ItemInstance` is hive-wide: an item a player owns follows them between the gameservers of one hive, so nothing about the *item* is keyed on a `GameServerId` — `RootGameServerId` records only *where an instance currently is*, resolved at delivery time, never who owns the row. (Corrected 2026-08-28: this used to say "and everything the inventory family touches", which phase 2 falsified. Two of the family's documents genuinely are per-server, and correctly so — `AppliedBatch` is keyed `{gameServerId}:{batchId}`, because one server's replay cache must never answer for another's or return another's batch, and `SuspiciousReconcile` records which server sent the refused reconcile. Both are *records about a gameserver's reporting*, not about item ownership, which is why they are keyed that way without contradicting the paragraph's point.) World-structure state, which lands later, is the opposite: a building's damage belongs to one map on one gameserver and means nothing on another. Splitting those into two modules now would buy nothing and cost a cross-module transaction (a single snapshot batch legitimately spans both), so they share a module and stay separated *inside* it — separate folders, separate repositories, separate endpoint groups (`/api/inventory/*` vs `/api/world/*`), no type shared between the two families. That is what keeps the later split cheap if per-server state ever outgrows the arrangement. Recorded here because the alternative is a future reviewer noticing one module serving two tenancy models and "fixing" it into two.
...
**Convention: an enum whose ordinal is persisted is append-only, and writes its numbers out.** Marten is configured through `Shared.Infrastructure/MartenPrivateSetterExtensions.UseSystemTextJsonWithPrivateSetters()`, whose `configure` callback sets only `TypeInfoResolver`. No `JsonStringEnumConverter` is registered anywhere in the solution and Marten's `EnumStorage` is never set, so **System.Text.Json writes every enum into JSONB as a bare integer ordinal** — in event payloads, which are immutable and replayed forever, and in inline-projected documents, which are rewritten from those events. The HTTP surface is unaffected and stays readable: enums cross the wire as *strings*, hand-parsed per endpoint with `Enum.TryParse` paired with `Enum.IsDefined` (`Enum.TryParse` alone also accepts a numeric string, which is why the pair matters), and `openapi/eliferpg-api-v1.json` accordingly contains no enum schemas at all. The ordinal is therefore invisible everywhere except in Postgres — which is exactly what makes it easy to break.

The rule that follows: any enum reachable from a Marten document or event is **append-only** — never insert, remove or reorder a member, only append at the end — and **must state its discriminants explicitly** (`Active = 0, Locked = 1`) even though C# would assign the same numbers implicitly. The explicit values buy nothing at runtime; they are there so that reordering members becomes a visible edit to a value in review rather than an invisible renumbering of every stored row, and so a reader can see the stored representation without counting lines. Enums that are *not* persisted — `SessionStatus`, `SkillCategory`, `AckKind`, all computed per request — deliberately keep implicit numbering, which leaves "has explicit values" a usable signal that an enum's numbers are load-bearing.

Two corollaries. **Choose member `0` deliberately**, because an event replayed from before a field existed binds to it: `ItemPersistence` orders `Despawns = 0` ahead of `Persistent = 1` on the reasoning that "a dropped bandage eventually disappears" is a far safer wrong answer than "a player's parked truck lives forever" would be in reverse. And **a `[Flags]` enum freezes bits, not positions** — `CompanyPermissions` is persisted on `CompanyPosition.Permissions` and read by `HasFlag` in three modules' authorization checks, so reassigning a bit silently regrants or revokes it on every existing position.

`ItemOrigin` is the reference example of the declaration shape. `SnapshotRejectionReason` carries the worked failure case: a stored `AppliedBatch`, written before a mid-list insert and replayed after it well within its 24-hour retention window, deserializes with a *different* rejection reason than the one it actually recorded. Note that nothing enforces any of this at build or test time — the existing enum tests (`SnapshotScopeKind_DeclaresNoServerWideMember`, the catalog-coverage tests) pin *membership*, not values — so it rests on the explicit numbers and on review.
